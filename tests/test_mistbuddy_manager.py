...
@pytest.mark.asyncio
async def test_start_mistbuddy_success(mistbuddy_manager):
    with patch('src.service.mistbuddy_manager.PowerBuddy') as mock_power_buddy:
        ...
        await mistbuddy_manager.start_mistbuddy("tent_one", 30)
...
@pytest.mark.asyncio
async def test_start_mistbuddy_value_error(mistbuddy_manager):
    with patch('src.service.mistbuddy_manager.PowerBuddy', side_effect=ValueError):
        with pytest.raises(HTTPException) as exc_info:
            await mistbuddy_manager.start_mistbuddy("invalid_tent", 30)
        assert exc_info.value.status_code == 400
        assert "Invalid input parameters" in str(exc_info.value.detail)

@pytest.mark.asyncio
async def test_start_mistbuddy_unexpected_error(mistbuddy_manager):
